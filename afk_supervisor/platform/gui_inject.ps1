...
# 统一 stdout 为 UTF-8: Windows PowerShell 5.1 在被重定向输出时按
# [Console]::OutputEncoding 编码字符串, 默认是系统 OEM 代码页 (本机 CP936),
# 于是中文回执到了 Python 侧就成了乱码, 连 hwnd / reason 一起被读崩。
# 宿主没有控制台句柄时 setter 可能抛异常, 所以两步都单独兜住:
# Python 侧 decode_powershell_output 仍能按旧代码页择优解码。
...
public class GuiInjector {
    ...
    // 完整性级别探测: 目标进程以管理员等高权限运行时, 低权限进程连
    // PROCESS_QUERY_INFORMATION 都打不开, UIA 只能看到宿主节点。
    ...
    public static IntPtr FindBestTargetWindow() {
        ...
        // 后台线程: 即使窗口枚举卡在异常窗口上, 超时后也不阻止进程退出。
        t.IsBackground = true;
        t.Start();
        t.Join(5000);
    ...
    // 最近一次身份校验时看到的 UIA 节点数: 用于区分"窗口没暴露自动化树"与"选错了目标任务"。
    public static volatile int LastDomNodes = -1;
    public static volatile int LastNamedNodes = -1;
    public static volatile string LastDomSample = "";
    // 诊断输出: 默认静默, 需要排查身份校验细节时设置 AFK_GUI_TRACE=1 再运行。
    public static readonly bool TraceEnabled =
        Environment.GetEnvironmentVariable("AFK_GUI_TRACE") == "1";

    // 只读自检: 枚举窗口 + UIA 节点普查 + 目标任务身份校验; 不敲键、不点按、不写剪贴板。
    public static string Probe(IntPtr hwnd, string targetSid, string targetTitle = "", int timeoutMs = 12000) {
        string result = "{\"ok\":false,\"error\":\"Probe did not complete\"}";
        Thread t = new Thread(() => {
            try {
                ...
                bool restored = false;
                if (iconic || !visible) {
                    // 最小化/隐藏会冻结渲染进程的可访问性树: 先恢复窗口再校验,
                    // 否则只会得到"树未暴露"的假结论 (与 Inject/PauseOrStop 的恢复动作一致)。
                ...
                AutomationElement node = VerifiedTaskRoot(hwnd, targetSid, targetTitle);
                if (node == null && restored) {
                    // 恢复后自动化树可能仍在异步建立: 再给渲染进程一次机会。
                    Thread.Sleep(600);
                    node = VerifiedTaskRoot(hwnd, targetSid, targetTitle);
                }
                bool identityFound = node != null;
                string identityDetail = "";
                if (identityFound) {
                    try {
                        identityDetail = "aid=" + (node.Current.AutomationId ?? "") +
                                         " help=" + (node.Current.HelpText ?? "");
                    } catch {}
                }
                string err = "";
                // reason 是稳定机读码: Python 侧据此给出可读结论, 不解析中文。
                string reason = identityFound ? "IDENTITY_OK" : "TARGET_NOT_FOCUSED";
                long procOpenError = 0;
                if (!identityFound && LastDomNodes <= 2) {
                    IntPtr hProc = OpenProcess(0x0400, false, pid);  // PROCESS_QUERY_INFORMATION
                    if (hProc != IntPtr.Zero) {
                        CloseHandle(hProc);
                    } else {
                        procOpenError = Marshal.GetLastWin32Error();
                    }
                    if (procOpenError == 5) {
                        reason = "UIA_ACCESS_DENIED";
                        err = "无法读取桌面端进程(错误 5=拒绝访问): 目标进程以更高权限运行(例如管理员), " +
                              "与当前守护进程完整性级别不一致, UIA 读不到它的可访问性树。请以相同或更高权限启动本守护进程, " +
                              "或让桌面端以普通权限重启。";
                    } else {
                        reason = "UIA_TREE_UNAVAILABLE";
                        err = "UIA 自动化树未暴露: 目标窗口只返回了 " + LastDomNodes +
                              " 个宿主节点, 桌面端渲染进程的可访问性树未开启" +
                              (restored ? " (已自动恢复最小化窗口后重试)" : "");
                    }
                } else if (!identityFound) {
                    err = "未在窗口内找到目标任务节点 (dom_nodes=" + LastDomNodes +
                          ", named_nodes=" + LastNamedNodes + "); 可能当前打开的是别的会话";
                }
                result = "{\"ok\":" + (identityFound ? "true" : "false") +
                         ",\"hwnd\":" + (long)hwnd + ",\"pid\":" + pid +
                         ",\"title\":\"" + title.ToString().Replace("\"", "'") + "\"" +
                         ",\"visible\":" + (visible ? "true" : "false") +
                         ",\"minimized\":" + (iconic ? "true" : "false") +
                         ",\"restored\":" + (restored ? "true" : "false") +
                         ",\"dom_nodes\":" + LastDomNodes +
                         ",\"named_nodes\":" + LastNamedNodes +
                         ",\"identity_found\":" + (identityFound ? "true" : "false") +
                         ",\"reason\":\"" + reason + "\"" +
                         ",\"proc_open_error\":" + procOpenError +
                         ",\"identity\":\"" + identityDetail.Replace("\"", "'").Replace("\\", "/") + "\"" +
                         ",\"dom_sample\":\"" + LastDomSample.Replace("\"", "'").Replace("\\", "/") + "\"" +
                         ",\"error\":\"" + err.Replace("\"", "'") + "\"}";
            } catch (Exception ex) {
                result = "{\"ok\":false,\"identity_found\":false,\"reason\":\"PROBE_ERROR\",\"error\":\"" + ex.GetType().Name + ": " +
        ...
        t.IsBackground = true;
        t.Start();
        if (!t.Join(timeoutMs)) { return "{\"ok\":false,\"identity_found\":false,\"reason\":\"PROBE_TIMEOUT\",\"error\":\"身份探针超时\"}"; }
        return result;
    }
    ...
    public static bool IsActiveTaskIdentity(string targetSid, string automationId,
        ...
        bool isOffscreen, string targetTitle = "") {
        if (String.IsNullOrWhiteSpace(targetSid) || isOffscreen) return false;
        string cleanSid = CleanSid(targetSid);
        if (String.IsNullOrWhiteSpace(cleanSid)) return false;
        if (String.Equals(automationId, "active-thread-" + cleanSid,
        ...
        if (visibleDocument && (
                String.Equals(automationId, "thread-" + cleanSid, StringComparison.OrdinalIgnoreCase) ||
                String.Equals(helpText, "codex://threads/" + cleanSid, StringComparison.OrdinalIgnoreCase))) {
            return true;
        }
        // Codex may expose only the stable eight-character short ID in DOM metadata.
        // Such a short marker is accepted only when the same node carries the exact title.
        if (cleanSid.Length <= 8 || String.IsNullOrWhiteSpace(targetTitle)) return false;
        string shortSid = cleanSid.Substring(0, 8);
        bool shortMarker = String.Equals(automationId, "active-thread-" + shortSid,
                                         StringComparison.OrdinalIgnoreCase) ||
                           String.Equals(automationId, "thread-" + shortSid,
                                         StringComparison.OrdinalIgnoreCase) ||
                           String.Equals(helpText, "codex://threads/" + shortSid,
                                         StringComparison.OrdinalIgnoreCase);
        return shortMarker && ExactTitleMatchesAny(name, targetTitle);
    }
    ...
    private static string NormalizeExactTitle(string value) {
        if (String.IsNullOrWhiteSpace(value)) return "";
        return value.Trim().TrimEnd('\u2026', '.').Trim();
    }

    public static bool ExactTitleMatches(string name, string targetTitle) {
        string cleanName = NormalizeExactTitle(name);
        string cleanTarget = NormalizeExactTitle(targetTitle);
        return !String.IsNullOrEmpty(cleanName) && !String.IsNullOrEmpty(cleanTarget) &&
               String.Equals(cleanName, cleanTarget, StringComparison.Ordinal);
    }

    // 同一个会话标题可能有多个来源 (侧栏显示名 / 简短标题 / 旧索引名)，字符串并不相同。
    // 传参统一用 Unit Separator (0x1F) 拼接为候选集合，逐个精确比较，
    // 仍然只在同一 SID 的机读标识前提下生效，不退回纯标题匹配。
    private static readonly char[] TitleSeparators = new char[] { (char)0x1F };

    public static string[] SplitTitles(string titles) {
        if (String.IsNullOrWhiteSpace(titles)) return new string[0];
        return titles.Split(TitleSeparators, StringSplitOptions.RemoveEmptyEntries);
    }

    public static bool ExactTitleMatchesAny(string name, string titles) {
        foreach (string candidate in SplitTitles(titles)) {
            if (ExactTitleMatches(name, candidate)) return true;
        }
        return false;
    }

    public static bool MatchesTitleAny(string docName, string titles) {
        foreach (string candidate in SplitTitles(titles)) {
            if (MatchesTitle(docName, candidate)) return true;
        }
        return false;
    }
    ...
    public static AutomationElement VerifiedTaskRoot(IntPtr hwnd, string targetSid, string targetTitle = "") {
        ...
        LastDomNodes = all.Count;
        int namedCount = 0;
        StringBuilder sample = new StringBuilder();
        foreach (AutomationElement n in all) {
            try {
                string aid = n.Current.AutomationId ?? "";
                string help = n.Current.HelpText ?? "";
                if (aid.Length == 0 && help.Length == 0) { continue; }
                namedCount++;
                if (sample.Length < 240 && namedCount <= 6) {
                    sample.Append(aid.Length > 0 ? aid : help).Append(" | ");
                }
            } catch {}
        }
        LastNamedNodes = namedCount;
        LastDomSample = sample.ToString().TrimEnd(' ', '|');
        ...
        foreach (AutomationElement node in all) {
            try {
                if (node.Current.ControlType == ControlType.Document && !node.Current.IsOffscreen) {
                    ...
                    if (!String.IsNullOrWhiteSpace(targetTitle) && MatchesTitleAny(docName, targetTitle)) {
                        matchedDoc = node;
                        break;
                    }
                    if (fallbackDoc == null) {
            ...
            foreach (AutomationElement node in docNodes) {
                try {
                    if (!node.Current.IsOffscreen) {
                        ...
                        if (MatchesTitleAny(name, targetTitle)) {
    ...
    public static string Inject(IntPtr hwnd, string text, string targetSid, string targetTitle = "", int timeoutMs = 8000) {
                ...
                if (!textSet) {
                    try {
                        ...
                        foreach (AutomationElement ae in freshEdits) {
                            try {
                                if (ae.Current.IsOffscreen || !ae.Current.IsEnabled) { continue; }
                                string fcls = ae.Current.ClassName ?? "";
                                string fname = ae.Current.Name ?? "";
                                bool looksLikeComposer = fcls.Contains("ProseMirror") || fname.Contains("ChatGPT") ||
                                    fname.Contains("\u6211\u4eec\u8981\u505a\u51fa\u4ec0\u4e48") || fname.Contains("Message") ||
                                    fname.Contains("\u8bf7\u8f93\u5165") || fname.Contains("Prompt");
                                if (!looksLikeComposer) { continue; }
                                object vp2;
        ...
        // Background so a stuck UI call cannot block process exit; never Thread.Abort
        // (unsupported on .NET Core, unsafe on Framework). The receipt stays UNCERTAIN.
        t.SetApartmentState(ApartmentState.STA); t.IsBackground = true; t.Start();
        if (!t.Join(timeoutMs)) { return "{\"ok\":false,\"delivery_status\":\"UNCERTAIN\",\"error\":\"UI operation timed out\"}"; }
        return result;
    }
    ...
    public static string PauseOrStop(IntPtr hwnd, string targetSid, string targetTitle = "", int timeoutMs = 8000) {
        ...
        t.SetApartmentState(ApartmentState.STA); t.IsBackground = true; t.Start();
        if (!t.Join(timeoutMs)) { return "{\"ok\":false,\"error\":\"Pause timed out\"}"; }
        return result;
    }
...
if ($ProbeOnly) {
    $res = [GuiInjector]::Probe([IntPtr]$TargetHwnd, $TargetSid, $TargetTitle, [int]$TimeoutMs)
    Write-Output $res
    exit 0
}
